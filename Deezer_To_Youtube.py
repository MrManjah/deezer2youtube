...
YOUTUBE_PLAYLIST_TITLE = "Import Deezer"
...
def get_existing_videos(youtube, playlist_id):
    ...
    while True:
        ...
        for item in response.get("items", []):
            existing_videos.add(item["snippet"]["title"].lower())
...
def search_and_add_tracks(youtube, playlist_id, tracks):
    ...
    for track in tracks:
        if track.lower() in existing_videos:
            print(f"Déjà présent : {track}")
            continue
        
        try:
            if track in search_cache:
                video_id = search_cache[track]
            else:
                search_request = youtube.search().list(q=track, part="snippet", maxResults=1, type="video")
                search_response = search_request.execute()
                if search_response["items"]:
                    video_id = search_response["items"][0]["id"]["videoId"]
                    search_cache[track] = video_id
                else:
                    print(f"Non trouvé : {track}")
            ...
            time.sleep(2)  # Pause pour éviter le dépassement de quota
        
        except googleapiclient.errors.HttpError as e:
            error_msg = str(e)
            if "quotaExceeded" in error_msg:
                print("Quota dépassé, enregistrement des titres restants et arrêt du script.")
                pending_tracks.extend(tracks[tracks.index(track):])
                break
            print(f"Erreur lors de l'ajout de {track} : {e}")
            time.sleep(5)
